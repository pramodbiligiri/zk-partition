...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    //TODO: ZKP: Change this so that outstanding changes for different paths are handled
    //concurrently
...
//        new HashMap<String, ChangeRecord>();
    ...
    //ZKP: To support multiple chains
	private HashMap<String, RequestProcessor> rpcMap = new HashMap<String, RequestProcessor>();
	private RequestProcessor defaultRequestProcessorChain;
    ...
    public ZooKeeperServer(FileTxnSnapLog txnLogFactory, int tickTime,
        ...
        this(txnLogFactory, tickTime, -1, -1, treeBuilder,
                new ZKDatabaseMulti(txnLogFactory));
    }
    ...
    public void dumpConf(PrintWriter pwriter) {
        ...
        pwriter.println(zkDb.getSnapLog().getSnapDir().getAbsolutePath());
        pwriter.print("dataLogDir=");
        pwriter.println(zkDb.getSnapLog().getDataDir().getAbsolutePath());
    ...
    throws IOException, InterruptedException {
        ...
        if (zkDb == null) {
            zkDb = new ZKDatabaseMulti(this.txnLogFactory);
        }  
        
//        if (!zkDb.isInitialized()) {
        	//TODO: ZKP: Load data is turned off
//            loadData();
//        }
    }
    ...
    public void startup() {        
        ...
        //TODO: ZKP: Turning off JMX for now
//        registerJMX();
    ...
    protected void setupRequestProcessors() {
    	LOG.info("Setting up request processors");
//    	defaultRequestProcessorChain = new RequestProcessorChain(this).start();
    	defaultRequestProcessorChain = createChain().start();
    }
    ...
    public void submitRequest(Request si) {
    	...
    	//TODO: ZKP: check request path, forward to corresponding chain
    	//if request type is create, then pick the corresponding chain
    	//invoke the firstProcessor of that chain   	  	
    	    	
        if (getDefaultChain() == null) {
            synchronized (this) {
                try {
                    while (!running) {
                        wait(1000);
                        LOG.info("Waited to init defaultRequestProcessorChain");
                    }
        ...
        try {
            ...
            if (validpacket) {
       			getChain(si.getRootPath()).processRequest(si);
//                firstProcessor.processRequest(si);
    ...
    protected RequestProcessor getChain(String rootPath) {
    	if(rootPath.isEmpty()){
    		return getDefaultChain();
    	}
    	
    	if(!rpcMap.containsKey(rootPath)){
    		rpcMap.put(rootPath, createChain().start());
    	}    	
    	
    	return rpcMap.get(rootPath);
	}

	protected RequestProcessorChain createChain() {
		return RequestProcessorChain.forDefault(this);
	}
    ...
    public void processConnectRequest(ServerCnxn cnxn, ByteBuffer incomingBuffer) throws IOException {
        ...
        //TODO: ZKP: This if-block negated for now
        if (false && connReq.getLastZxidSeen() > zkDb.getDataTree().lastProcessedZxid) {
    ...
    protected RequestProcessor getDefaultChain() {
    	return defaultRequestProcessorChain;
    }
