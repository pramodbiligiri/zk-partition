...
public class PrepRequestProcessor extends Thread implements RequestProcessor {
	...
	private RequestProcessorChain chain;
    ...
    ChangeRecord getRecordForPath(String path) throws KeeperException.NoNodeException {
        ...
        synchronized (chain.getOutstandingChanges()) {
            lastChange = chain.getOutstandingChangesForPath().get(path);
            /*
                ...
                if (n != null) {
                    ...
                    lastChange = new ChangeRecord(-1, path, n.stat,
                        children != null ? children.size() : 0,
                            zks.getZKDatabase().convertLong(path, acl));
                }
            }
        }
    ...
    void addChangeRecord(ChangeRecord c) {
        synchronized (chain.getOutstandingChanges()) {
        	chain.getOutstandingChanges().add(c);
        	chain.getOutstandingChangesForPath().put(c.path, c);
        }
    }
    ...
    void rollbackPendingChanges(long zxid, HashMap<String, ChangeRecord>pendingChangeRecords) {
        ...
        synchronized (chain.getOutstandingChanges()) {
            ...
            ListIterator<ChangeRecord> iter = chain.getOutstandingChanges().listIterator(chain.getOutstandingChanges().size());
            while (iter.hasPrevious()) {
                ChangeRecord c = iter.previous();
                if (c.zxid == zxid) {
                    iter.remove();
                    chain.getOutstandingChangesForPath().remove(c.path);
                } else {
                    break;
                }
            }
           
            boolean empty = chain.getOutstandingChanges().isEmpty();
            long firstZxid = 0;
            if (!empty) {
                firstZxid = chain.getOutstandingChanges().get(0).zxid;
            }
        ...
        throws KeeperException, IOException, RequestProcessorException
    {
        ...
        switch (type) {
            case OpCode.create:                
                ...
                ChangeRecord parentRecord = null;
                if(!parentPath.isEmpty()){
	                //TODO: ZKP: Figure out why exactly parent path is being used here
	                //TODO: ZKP: How to handle returning the right db for parentPath?? :(
                ...
                //TODO: ZKP: Adding parentRecord null check for the case of empty parentPath
                ...
                int newCversion = parentCVersion + 1;
            ...
            case OpCode.closeSession:
                ...
                synchronized (chain.getOutstandingChanges()) {
                    for (ChangeRecord c : chain.getOutstandingChanges()) {
                        if (c.stat == null) {
                            // Doing a delete
                            es.remove(c.path);
	...
	public void setChain(RequestProcessorChain chain) {
		this.chain = chain;
	}
}
